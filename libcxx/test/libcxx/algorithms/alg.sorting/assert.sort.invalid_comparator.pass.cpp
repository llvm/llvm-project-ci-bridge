...
int main(int, char**) {
    ...
    std::set<std::size_t*> element_ptrs;
    for (std::size_t i = 0; i != comparison_results.size(); ++i) {
        elements.push_back(new std::size_t(i));
        element_ptrs.insert(elements.back());
    }
    ...
    auto checked_predicate = [&](size_t* left, size_t* right) {
        ...
        assert(element_ptrs.contains(left));
        assert(element_ptrs.contains(right));
        return predicate(left, right);
    };

    // Check the classic sorting algorithms
    {
        std::vector<std::size_t*> copy = elements;
