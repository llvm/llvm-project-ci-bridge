...
// Use sed instead of grep because some headers don't have transitive includes and grep fails when no input matches
// RUN{BLOCKLIT}: cat %{{libcxx}}/test/libcxx/transitive_includes/%{{cxx_std}}.csv | sed -rn 's#(^{header} )#\\1#p' > %t/expected_transitive_includes.csv
// RUN{BLOCKLIT}: diff %t/expected_transitive_includes.csv %t/actual_transitive_includes.csv
#include <{header}>
""")
