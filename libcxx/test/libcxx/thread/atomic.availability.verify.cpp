...
void f() {
    {
        ...
        std::atomic_wait(&i, 4); // expected-warning {{'atomic_wait' is only available on}}
        std::atomic_wait_explicit(&i, 4, m); // expected-warning {{'atomic_wait_explicit' is only available on}}
        std::atomic_notify_one(&i); // expected-warning {{'atomic_notify_one' is only available on}}
        std::atomic_notify_all(&i); // expected-warning {{'atomic_notify_all' is only available on}}
    }
