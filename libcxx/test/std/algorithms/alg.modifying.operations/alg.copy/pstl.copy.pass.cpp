...
struct TestInt {
  ...
  void operator()(Policy&& policy) {
    ...
    for (const int size : {0, 1, 2, 100, 350}) {
      ...
      decltype(auto) ret = std::copy(policy, Iter1(std::data(a)), Iter1(std::data(a) + std::size(a)), Iter2(std::data(out)));
...
struct TestIteratorsInt {
  ...
  void operator()() {
    types::for_each(types::forward_iterator_list<int*>{},
                    TestIteratorWithPolicies<types::partial_instantiation<TestInt, Iter2>::template type>{});
  }
};
...
struct TestIteratorsNonTrivial {
  ...
  void operator()() {
    types::for_each(types::forward_iterator_list<CopiedToTester*>{},
                    TestIteratorWithPolicies<types::partial_instantiation<TestNonTrivial, Iter2>::template type>{});
  }
};
