...
__insertion_sort_unguarded(_RandomAccessIterator __first, _RandomAccessIterator __last, _Compare __comp) {
  ...
  if (__first == __last)
    return;
  _RandomAccessIterator const __leftmost = __first - difference_type(1); (void)__leftmost; // can be unused when assertions are disabled
...
__bitset_partition(_RandomAccessIterator __first, _RandomAccessIterator __last, _Compare __comp) {
  ...
  if (__first < __last) {
    ...
    do {
      --__last;
      _LIBCPP_ASSERT(__last != __begin - difference_type(1), "Would read out of bounds, is your comparator a valid strict-weak ordering?");
